...
public class NewOpportunityService {
    ...
    public static void NewOportunityDeleteOperationService(int delete_seqnum) {
      ...
      for(int index = 0;index<delete_column.size();index++) { 
          String selectDeleteSeqQuery = "select seq_no from opportunity_info_details where column_name=?;"; 
          PreparedStatement st5 = connection.prepareStatement(selectDeleteSeqQuery);
          st5.setString(1,delete_column.get(index));
      ...
      try
      {
           ...
           while(rs2.next())
           {
               ...
               {
                   String Seq_Num_Query  = "Select * from Opportunity_Info_Details where column_name =?;";
                   PreparedStatement st3 = connection.prepareStatement(Seq_Num_Query);
                   st3.setString(1,column_name);
                   ResultSet rs3 = st3.executeQuery();
                   int seq_num = 0;
                   if(rs3.next())
                   {
           ...
           {
               String SelectedQuery = "select * from opportunity_info_details where column_name =?;";
               PreparedStatement st3 = connection.prepareStatement(SelectedQuery);
               st3.setString(1,col_name);
               ResultSet rs3 = st3.executeQuery();
               if(!rs3.next())
               {
                      String SelectDetailsQuery = "select * from opportunity_info_template_details where column_name=?;";
                  ...
                  prestmt.setString(9, tempmandatory);
                  prestmt.setString(10, value); 
                  prestmt.setString(11, umandatory);
                  prestmt.execute(); 
                  JsonObject jsonObj = new JsonObject(); 
                  ...
                  jsonObj.addProperty("Mandatory",tempmandatory);
                  jsonObj.addProperty("Value",value); 
                  jsonObj.addProperty("UMandatory",umandatory);
              ...
              while(rs.next())
              {
                  if(rs.getString("column_name").startsWith("OpportunityAddInfo"))
                  {
                      ...
                      if(!seqnum.equals(append_seq_num))
                      {
                        ...
                        String UpdateQuery = "Update Opportunity_Info_Details set column_name =? where seq_no=?;";  
