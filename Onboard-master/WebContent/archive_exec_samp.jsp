	  ...
	  function getID(pln_srt,pln_end,act_srt,status,pln_hrs,act_hrs,progressbar,actual_enddate)
	  {
		 // window.alert(pln_srt+" "+pln_end+" "+act_srt+" "+status.id+" "+pln_hrs.id+" "+act_hrs.id+" "+progressbar.id);
		  ...
		  $('#'+status.id).css({background:'green'});
			  ...
			  {
			  alert("Values should not be null");
			  }
		  }
	  </script>
	  <script>
...
<form class="form-signin" name="loginForm" method="post" action="archive_exec">
	...
	var plan_end=[];
	var e=0;
		function pass(current_name,current_value,current_id){
<%
...
//window.alert(current_name+" "+current_value+" "+current_id);
for(var j=0;j<e;j++)
{
	   if (task_name[j]==current_name)
	   {
		   var level_numb=level_number[j];
		   var sequence_numb=sequence_number[j];
		  var index=j;
		   var level1=level_numb;
		   for(var k=j-1;k>=0;k--)
  		   {
  			   if (level_number[k]==0)
  			   {
  			   ...
  			   {
  				   if(plan_start[k]<current_value)
  					   {
  					  // alert("hi");
  					 plan_start[k]=current_value;
  					 console.log(k);
  				// document.getElementById("pln_srt_date"+k).value=current_value;
    				   level1=level1-1;   
  					   }
  				   else
  					   {
     ...
     for(var x=0;x<e;x++)
    	 console.log(task_name[x]+" "+plan_start[x]);
}
</script>
<%
if(rs4.next()){
...
<td><input type="text" class="in" id="act_srt_date<%=i %>" name="act_srt_date<%=i %>" value="<%=rs3.getString(5) %>"  /></td>
<td><input  type="text" class="in" id="act_end_date<%=i %>" name="act_end_date<%=i %>" value="<%=rs3.getString(6) %>"  /></td>
<td><input  type="text" class="in" placeholder="enter" id="phours<%=i %>" name="hrs<%=i %>" value="<%=rs3.getString(9)%>" onclick= /></td>
<td><input  type="text" class="in" placeholder="enter" id="hours<%=i %>" name="hrs<%=i %>" value="<%=rs3.getString(9) %>" onclick="pass('<%=rs3.getString(3)%>',document.getElementById('pln_srt_date<%=i %>').value,'<%=i %>');getID(document.getElementById('pln_srt_date<%=i %>').value,document.getElementById('pln_end_date<%=i %>').value,document.getElementById('act_srt_date<%=i %>').value,document.getElementById('status<%=i %>'),document.getElementById('phours<%=i %>'),document.getElementById('hours<%=i %>'),document.getElementById('progressbar<%=i %>'),document.getElementById('act_end_date<%=i %>').value);" /></td>
