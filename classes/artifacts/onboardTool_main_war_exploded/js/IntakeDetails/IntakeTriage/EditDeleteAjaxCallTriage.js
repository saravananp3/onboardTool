    	...
    	$.ajax({
            ...
            success: function (data) {
            	...
            	if(data.mandatory==true)
            		{
            			$(".InputField").eq(index).find("label[for =triage] div:first").addClass("required_fie");
            			$(".InputField").eq(index).find("label[for =triage] div:first .hidedel").remove();
            			$(".InputField").eq(index).find("label[for =triage] div:first").append("<div class='deletealert' style='display:none;'></div>");
            		}
            	else
            		{
            			$(".InputField").eq(index).find("label[for =triage] div:first").removeClass("required_fie");
            			$(".InputField").eq(index).find("label[for =triage] div:first .deletealert").remove();
            			$(".InputField").eq(index).find("label[for =triage] div:first").append("<span class='glyphicon glyphicon-trash deletealert hidedel' style='float:right;display:none;'></span>");
            		}
            	var html = $(".InputField").eq(index).find("label[for =triage]").html();
            	var html1 =html.replace(previous_label_name,updated_label_name);
            	
            	console.log("HTML "+html1);
            	$(".InputField").eq(index).find("label[for =triage]:first").html(html1);
    ...
    function TriageDeleteAjax(Seq_Num)
    {
    	$.ajax({
            ...
            success: function (data) {
            	...
            	console.log($(".InputField").eq(index).remove());
